...
{
	void *pvBaseAddress;
	unsigned int ulLengthInBytes;
	unsigned int ulParameters;
} xMemoryRegion;
...
typedef struct xTASK_STATUS
{
	...
	unsigned int ulRunTimeCounter;				/* The total run time allocated to the task so far, as defined by the run time stats clock.  See http://www.freertos.org/rtos-run-time-stats.html.  Only valid when configGENERATE_RUN_TIME_STATS is defined as 1 in FreeRTOSConfig.h. */
 ...
 */
unsigned portBASE_TYPE uxTaskGetSystemState( xTaskStatusType *pxTaskStatusArray, unsigned portBASE_TYPE uxArraySize, unsigned int *pulTotalRunTime );
