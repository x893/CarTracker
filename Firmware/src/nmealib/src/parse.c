...
uint8_t _nmea_parse_time(const char *buff, int buff_sz, nmeaTIME *res)
{
    uint8_t success = 0;
	int hh, mm, ss, ms = 0;
 ...
 * @return 1 (true) - if parsed successfully or 0 (false) - if fail.
 */
uint8_t nmea_parse_GPRMC(const char *buff, int buff_sz, nmeaGPRMC *pack)
{
    int nsen;
    char time_buff[NMEA_TIMEPARSE_BUF];
    ...
    if (nsen == 13 || nsen == 14)
    {
		if (0 == _nmea_parse_time(&time_buff[0], strlen(time_buff), &(pack->utc)))
    	{
		    if (pack->utc.year < 90)
        		pack->utc.year += 2000;
    		pack->utc.mon -= 1;
...
void nmea_GPRMC2info(nmeaGPRMC *pack, nmeaINFO *info)
{
    ...
    info->utc = pack->utc;
    info->lat = ((pack->ns == 'S') ? -(pack->lat) : pack->lat);
    info->lon = ((pack->ew == 'W') ? -(pack->lon) : pack->lon);
